...
class ChatRoom:
    id = 1
    def __init__(self, title: str, persons: list | None = None, chatdatas: list | None = None):
        self.id = ChatRoom.id
        ChatRoom.id += 1
    ...
    def add_person(self, name: str, persona: str, is_user: bool = False, chatroom_id: int | None = None):
        person = Person(name=name, persona=persona, chatroom_id=chatroom_id, is_user=is_user)
...
def create_chatroom(title: str, persons_data: list) -> ChatRoom:
    ...
    for person_data in persons_data:
        chatroom.add_person(name=person_data["name"], persona=person_data["persona"], is_user=False, chatroom_id=chatroom.id)
    return chatroom
