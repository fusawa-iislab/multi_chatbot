...
export const PersonsInfoListItem: React.FC<{ person: PersonType, chatRoomId: number }> = ({ 
	person, 
	chatRoomId 
}) => {
			...
			<div className="text-right">
				{person.isUser ? (
					<button
						...
						onClick={() => { setTextareaIsOpen(!textareaIsOpen); }}
					>
			...
			{textareaIsOpen && person.isUser && (
				<div className="fixed bottom-0 left-0 right-0 w-screen flex flex-col items-center bg-gray-800 p-2">
					...
					<textarea
						...
						rows={3}
						onChange={(e)=> setUserMessage((e.target as HTMLTextAreaElement).value)}
						value={userMessage}
					/>
					<button 
